# -*- coding: utf-8 -*-
"""
Created on Wed Oct  1 22:50:56 2025
...
# Interface: cada classe tem forecast(df, periods, freq) -> DataFrame com colunas ['ds','yhat','yhat_lower','yhat_upper']
...
class ProphetProjection(Projection):
    def forecast(self, df_full, periods, freq):
        ...
        # parâmetros do prophet vindos do config
...
class ARIMAProjection(Projection):
    def forecast(self, df_full, periods, freq):
        ...
        # _get_train_df espera DataFrame com ['ds','y']
        ...
        # parâmetros de busca vindos do config
        ...
        idx_out = pd.date_range(start=start_out, periods=periods, freq=freq)
        ...
        df_out = pd.DataFrame({
            'ds': idx_out,
            'yhat': out_mean.values,
            'yhat_lower': out_ci.iloc[:,0].values,
            'yhat_upper': out_ci.iloc[:,1].values
        })
...
class HoltProjection(Projection):
    def forecast(self, df_full, periods, freq):
        ...
        df_out = pd.DataFrame({
            'ds': idx_out,
            'yhat': out.values,
            'yhat_lower': out.values - z*sigma,
            'yhat_upper': out.values + z*sigma
        })
        df_full_forecast = pd.concat([df_in, df_out], ignore_index=True).sort_values('ds').reset_index(drop=True)
        return df_full_forecast